...
DATA_DIR = r'G:\sj459\smart-heat-main\smart-heat-main\data'
# DATA_DIR = '../data'
...
def get_counties():
    ...
    for n, ind in enumerate(meta_data.index):
        ...
        if meta_data.loc[ind, 'Country'] == 'US':
            ...
            # Process counties in California
            elif meta_data.loc[ind, 'ProvinceState'] == 'CA':
                if meta_data.loc[ind, 'ProvinceState'] == 'CA':
                    address = {'city': meta_data.loc[ind, 'City'], 'state': 'California'}
...
def get_solar_dfs(location, index):
    ...
    solar_dfs_loc = [pd.read_csv(f'{DATA_DIR}/Weather/{location}/{f}', nrows=1).loc[0, ['Latitude', 'Longitude']] for f
...
def import_grouped_data(location: str,
    ...
    def import_data(i, file, reduce_size):
        ...
        if READ_IN:
            ...
            elif building_meta_data['ProvinceState'].iloc[0] == 'CA':
                address = {'city': building_meta_data['City'].iloc[0], 'state': 'California'}
            else:
